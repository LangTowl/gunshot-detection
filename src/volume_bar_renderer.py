...
def volume_animation_bar(stop_event, volume_queue, prediction, samples_sniffed, gunshots_detected, average_volume, last_volume, chunk_duration = 0.01, max_length=40):
    ...
    while not stop_event.is_set():
        try:
            ...
            sys.stdout.write(f"\r|{bar}| {percentage} [Prediction: {prediction.value:.5f}] [Samples Sniffed: {samples_sniffed.value:5d}] [Gunshots Detected: {gunshots_detected.value}] [Avg. Volume: {average_volume.value:.3f}] [Last Volume: {last_volume.value:.3f}]")
